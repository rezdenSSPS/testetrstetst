...
function App() {
  ...
  const handleLoanSubmit = async (itemId: string, personId: string, quantity: number, notes: string) => {
    try {
      await createLoan(itemId, personId, quantity, notes);
  ...
  const handleLoanReturn = async (loanId: string) => {
    try {
      ...
      if (loan) {
        ...
        await updateItemQuantity(loan.item_id, -loan.quantity);
